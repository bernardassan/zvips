...
pub fn callT(func: anytype, comptime Option: type, valist: Option) @typeInfo(@TypeOf(func)).@"fn".return_type.? {
    std.debug.assert(@typeInfo(@TypeOf(valist)).@"struct" == true);
    const valist_tuple = comptime flatTuple(Option, valist);
    return @call(.auto, func, valist_tuple);
}
